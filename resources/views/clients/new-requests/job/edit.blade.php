...
@section('content')
    ...
    <main class="section">
        ...
        {{ Form::model($job, [ 
                'route' => [ $client->rejected ? 'clients.destroy' : 'clients.new-requests.job.update', $client->id ], 
            ...
            @includeWhen(get_class($job) == 'App\InterpreterJob', 'interpreter-jobs.form', ['interpreterJob' => $job, 'hideSubmit' => true])

            @includeWhen(get_class($job) == 'App\TranslatorJob', 'translator-jobs.form', ['translatorJob' => $job, 'hideSubmit' => true])
